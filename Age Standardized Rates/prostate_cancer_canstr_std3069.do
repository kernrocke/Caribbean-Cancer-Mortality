cls
***An age-standardization do file used to populate table 1(adults 30-69 years)
...
log using "Log files\Age Standardization\prostate_cancer_canstr_std3069", replace

**  GENERAL DO-FILE COMMENTS
**  program:		prostate_cancer_canstr_std3069.do
**  project:      	Caribbean Cancer mortality trends 
**  author:       	Rocke&Sobers \ 26-AUG-2016 \ revised 14-OCT-2018
**  task1:          create age-standardised prostate cancer rates for adults 30-69
...
*In this section I am creating a loop for the creation of graphs
	...
	export excel using "G:\Statistical Data Anlaysis\2018\Cancer Mortality Trends\Data\Mortality Rates\Standardized_3069\Prostate\Prostate_data30_`x'.xls", replace firstrow(variables)	
		keep if sex==2
	gen year10 = year/10
	
	
	lowess rateadj year if cod == 61, gen(lows_`x')nog
	
	dis as text "POISSON BASED ON COUNTRY = " "`x'"

	
	poisson rateadj if cod == 61, irr
	
	predict preds_`x', ir
	
	sort year
	
	** Graph --> Adjusted Prostate Cancer mortality rate for 21 Caribbean countries (men)
#delimit ;
	graph twoway 	
		/// Lowess smoother Prostate
		...
		lab(1 "Smoothed Prostate rate") 
		lab(2 "Poisson model Prostate")
		);
		
#delimit cr

graph save Graph "G:\Statistical Data Anlaysis\2018\Cancer Mortality Trends\Data\Mortality Rates\Standardized_3069\Prostate\graph_Prostate_`x'.gph", replace
