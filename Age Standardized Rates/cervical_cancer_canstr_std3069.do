cls
***An age-standardization do file used to populate table 1(adults 30-69)
...
log using "Log files\Age Standardization\cervical_cancer_canstr_std3069", replace

**  GENERAL DO-FILE COMMENTS
**  program:		cervical_cancer_canstr_std3069.do
**  project:      	Caribbean Cancer mortality trends 
**  author:       	Rocke&Sobers \ 26-AUG-2016 \ revised 14-OCT-2018
**  task1:          create age-standardised cervical cancer rates for adults 30-69
...
keep if cod ==61 | cod==50 | cod==18 | cod==34 | cod==16 | cod==53
...
foreach x of local levels2 {
	...
	distrate deaths pop2 using "G:\Statistical Data Anlaysis\2018\Cancer Mortality Trends\Data\Population Numbers\world_pop3.dta" if country4 == "`x'" , stand(age5) popstand(pop_std) by(year sex cod) mult(100000) format(%8.2f) ///
...
*In this section I am creating a loop for the creation of graphs
	...
	export excel using "G:\Statistical Data Anlaysis\2018\Cancer Mortality Trends\Data\Mortality Rates\Standardized_3069\Cervical\Cervical_data3069_`x'.xls", replace firstrow(variables)	
		keep if sex==1
	gen year10 = year/10
	
	
	lowess rateadj year if cod == 18, gen(lows_`x')nog
	
	dis as text "POISSON BASED ON COUNTRY = " "`x'"

	
	poisson rateadj if cod == 18, irr 
	
	predict preds_`x', ir
	
	sort year
	
	** Graph --> Adjusted Cervical Cancer mortality rate for 21 Caribbean countries (women)
#delimit ;
	graph twoway 	
		/// Lowess smoother Cervical
		(line lows_`x' year if sex == 1, clw(0.25) clc(gs12) clp("l") )
		/// Poisson  --> Cervical
		(line preds_`x' year if sex == 1, clw(0.25) clc(green*0.5) clp("l") )
		,
		...
		lab(1 "Smoothed Cervical rate") 
		lab(2 "Poisson model Cervical")
		);
		
#delimit cr

graph save Graph "G:\Statistical Data Anlaysis\2018\Cancer Mortality Trends\Data\Mortality Rates\Standardized_3069\Cervical\graph_Cervical_`x'.gph", replace
